...
#ifndef _nofont_16x16	// tBUG Optional removal to save code space
#include "font16x16.h"
#endif
...
#endif
#ifndef _nofont_16x16	// tBUG
	else if(oledFont == FONT16X16){
    ci = c - 128;
		if (oledX > 128 - 16)
		{
			oledX = 0;
		...
		for (i = 0; i < 16; i++)
		{
			Wire.write(pgm_read_byte(&ssd1306xled_font16x16[ci * 32 + i]));
		}
		ssd1306_send_data_stop();
		setCursor(oledX, oledY + 1);
		ssd1306_send_data_start();
		for (i = 0; i < 16; i++)
		{
			Wire.write(pgm_read_byte(&ssd1306xled_font16x16[ci * 32 + i + 16]));
		}
		ssd1306_send_data_stop();
		setCursor(oledX + 16, oledY - 1);
	}
#endif
	return 1;
}
